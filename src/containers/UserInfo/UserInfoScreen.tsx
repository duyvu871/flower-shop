...
function UserInfoScreen({}: UserInfoScreenProps) {
	...
	useEffect(() => {
		...
		setNewTelegramLink(userData.telegram);
	}, [userData]);
				...
				<div className={'flex flex-col justify-center items-start gap-2'}>
					...
					<Input
						...
						classNames={{
							input:
								userData.balance < 0
									? 'text-danger-500 font-semibold text-lg'
									: 'text-success-500 font-semibold text-lg',
						}}
						...
						endContent={
							<span
								className={tw(
									'p-1 rounded text-white text-xl cursor-pointer',
									userData.balance < 0
										? 'bg-danger-500 hover:bg-danger-600'
										: 'bg-success-500 hover:bg-success-600',
								)}
								onClick={() => push('/buy-credit')}>
								<LuPlus />
							</span>
						}
					/>
