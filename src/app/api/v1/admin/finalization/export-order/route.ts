...
export async function GET(req: NextRequest) {
    ...
    const orderResult = allOrders.map((order, index) => {
        return {
            ...
            "Số đơn": order.orderList.reduce(
                (acc, cur) => acc + cur.totalOrder,
                0,
            ),
            "Danh sách món": order.orderList
                .map((item) => item.name + " " + item.totalOrder)
                .join("\n"),
            "Tổng tiền": order.orderVolume,
            "Ghi chú": order.note || "",
            "Địa chỉ": order.location || "",
            "Trạng thái":
                order.status === "approved" ? "Đã duyệt" : "Chưa duyệt",
    ...
    headers.append(
        "Content-Disposition",
        'attachment; filename="thongtindonhang-' + TimeRange[range] + '.xlsx"',
    );
