...
export async function GET(req: NextRequest) {
    ...
    const purchaseCollection = client.db(process.env.DB_NAME).collection("purchase-orders");

    const allOrders = await purchaseCollection.find({
        createdAt: {
            $gte: timeStart
        }
    }).toArray();
    ...
    const buffer = await exportService.exportDataForVisualization(orderResult, "naptien");

    const headers = new Headers();
    headers.append("Content-Disposition", 'attachment; filename="thontinnaptien-'+ TimeRange[range] +'.xlsb"');
