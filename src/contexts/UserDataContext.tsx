...
export const UserDataProvider = ({ children }: { children: ReactNode }) => {
    ...
    const [userData, setUserData] =
        useState<WithId<UserInterface>>(defaultUserData);
    const [userWithdrawalHistory, setUserWithdrawalHistory] = useState<
        OrderType[]
    >([]);
    const [isLoaded, setIsLoaded] = useState<boolean>(false);
    // update user data
    const updateFullUserData = async function (
        data: Partial<WithId<UserInterface>>,
    ) {
    ...
    const updateBankingMethod = useCallback(
        async function (bankMethod: BankingMethodUpdate) {
            const res = await fetch(
                "/api/v1/auth/update/update-banking-method",
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
    ...
    // update password
    const updatePassword = useCallback(async function (
        oldPassword: string,
        newPassword: string,
    ) {
