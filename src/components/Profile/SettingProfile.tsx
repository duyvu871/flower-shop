...
import { TbClockDollar, TbMoneybag } from 'react-icons/tb';
...
function SettingProfile({}: SettingProfileProps) {
	...
	const handler = [
		...
		{
			iconStart: <TbMoneybag size={25} className={'text-green-500'} />,
			title: 'Ký quỹ nợ',
			action: () => {
				push('/profile/debit-margin');
			},
			type: 'action',
		},
		{
		...
		},
		{
			...
			action: () => {
				push('/profile/thong-tin-ca-nhan');
			},
			type: 'action',
		},
		{
