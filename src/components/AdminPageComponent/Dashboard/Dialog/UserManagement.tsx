...
function UserManagement({ _id }: UserManagementProps) {
	...
	const handleUpdate = async () => {
		...
		store.dispatch(
			updateUsers(
				data.users.map(item =>
					(item._id as unknown as string) === _id
						? {
								...item,
								address: newAddress,
								phone: newPhone,
		...
		<>
			<div className={'flex flex-col justify-center items-center gap-4'}>
							...
							<div className={'w-full flex flex-col justify-center items-start gap-2'}>
								...
								<Input
									...
									endContent={
										<span
											className={
												'p-1 rounded bg-danger-500 text-white text-xl hover:bg-danger-600 cursor-pointer'
											}
											onClick={() => {
				...
				<div className={'flex flex-row gap-2'}>
					...
					<Button className={'bg-red-500 text-white'} onClick={() => setOpenConfirmDelete(true)}>
						Xóa
					</Button>
				</div>
			</div>
