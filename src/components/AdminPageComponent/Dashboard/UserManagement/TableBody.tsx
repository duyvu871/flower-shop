			...
			{data.map((user, data_index) => (
				<tr className={''} key={'row' + data_index}>
					{keys.map((key, index) => {
						...
						if (key === 'balance') {
							const isShowVirtualVolume = user['balance'] <= 0;
							return (
								<td key={'td' + index} className={'px-6 py-4 whitespace-nowrap text-base'}>
									<span
										className={tw(
						...
						if (key === 'telegram') {
							return (
								<td key={'td' + index} className={'px-6 py-4 whitespace-nowrap text-base w-24'}>
									<Link href={user[key] as string} isExternal={true}>
										{user[key]}
									</Link>
								</td>
							);
						...
						return (
							<td key={'td' + index} className={'px-6 py-4 whitespace-nowrap text-base'}>
								{actions[index] === 'formatCurrency' ? (
									<span className={'font-semibold'}>{formatCurrency(user[key].toString())}</span>
								) : (
									<span>{user[key]}</span>
								)}
							</td>
