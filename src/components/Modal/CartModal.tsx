...
function CartModal({}: CartModalProps) {
	...
	const handlePayToBill = () => {
		...
		storeToLocalStorage('isPayAll', isPayAll);
		push('/order');
	...
	const handleRemoveFromCart = (itemIds: string[]) => {
		...
		setDeleteItems(prev => {
			return cart.reduce((acc, item, index) => {
				if (item?.delete_or_select === undefined || itemIds.length === 0) {
					cart[index].delete_or_select =
						cart[index].delete_or_select === undefined ? true : cart[index].delete_or_select;
					handleDeleteList(item._id as unknown as string, cart[index].delete_or_select);
				}
				return {
					...acc,
					[item._id as unknown as string]:
						item?.delete_or_select !== undefined ? item.delete_or_select : true,
				};
			}, {});
		});
	};
	...
	useEffect(() => {
		setOrderQuantity(prev => {
			return cart.reduce((acc, item) => {
				return {
					...acc,
					[item._id as unknown as string]: item.totalOrder,
				};
			}, {});
		});
		...
		if (selectedItems.length !== 0) {
			setDeleteItems(prev => {
				return selectedItems.reduce((acc, item) => {
					return {
						...acc,
						[item._id as unknown as string]: !!item.delete_or_select,
					};
				}, {});
			});
	...
	useEffect(() => {
		// if (!isCartModalOpen) {
		const checkedItems = Object.entries(deleteItems)
	...
	return (
		<Modal
			...
			<ModalContent>
				{onClose => (
					<>
						<ModalHeader className={'flex flex-col gap-1'}>Giỏ hàng của bạn ({cart.length})</ModalHeader>
							...
							{cart.map((item, index) => {
								return (
									<CartItem
										...
										minusTotalOrder={minusTotalOrder}
										orderValueRef={orderValueRef}
										orderQuantity={orderQuantity}
										plusTotalOrder={plusTotalOrder}
									/>
								);
							})}
						</ModalBody>
							...
							<div className={'flex flex-col justify-center items-start w-full gap-2'}>
								<div className={'flex flex-row justify-between gap-2'}>
									...
									<span className={'font-bold'}>
										{formatCurrency(totalPayment.toString()).replaceAll(',', '.')}
										.000đ
									</span>
								</div>
								<div className={'flex justify-between items-center w-full'}>
									<Button className={'bg-orange-600 text-white'} onClick={handlePayToBill}>
										Thanh toán
									</Button>
									<Button
										...
										className={tw(
											...
											isPreventDelete ? 'bg-gray-400 cursor-not-allowed' : '',
										)}
										onClick={() =>
											handleRemoveFromCart(Object.keys(deleteItems).filter(key => deleteItems[key]))
										}>
										Xóa
									</Button>
								</div>
