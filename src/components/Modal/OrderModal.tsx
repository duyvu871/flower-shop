...
function OrderModal({}: OrderModalProps) {
	...
	const { findItem, addToCart, getItemById, storeItemToLocalStorage } =
		useMenuData()
	const { isOrderModalOpen, orderId } = useSelector(
	...
	useEffect(() => {
		setPrice(
			Math.floor(
				orderInfo.price -
					Number(
						calculateDiscount(String(orderInfo.price), orderInfo.discount || 0),
					),
			),
		)
	}, [orderInfo])
				...
				{onClose => (
					<>
						...
						<ModalBody className={'flex flex-col gap-1'}>
							...
							/>
							<div
								className={'flex flex-col gap-1 border-gray-200 border-b-[5px]'}
							>
									...
									<div className={'flex flex-col'}>
										...
										<div className={'flex flex-row gap-2 mb-2'}>
											<p className={'line-through text-gray-400 text-sm'}>
												{orderInfo.price}.000đ
											</p>
											<p className={'text-gray-400 text-sm'}>
												{orderInfo.total_sold} đã bán
											</p>
										</div>
									</div>
									<div
