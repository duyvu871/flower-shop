...
function OrderModal({}: OrderModalProps) {
	...
	const handleImmediately = () => {
		...
		store.dispatch(closeOrderModal());
		push(
			'/order?immediately=true&order_id=' +
				orderInfo._id +
				'&total_order=' +
				totalOrder +
				'&take_note=' +
	...
	useEffect(() => {
		...
		if (item) {
			...
			if (orderId) {
				getItemById([orderId] as string[]).then(item => {
					// console.log(item)
					if (item) {
						setOrderInfo(item[0]);
	...
	useEffect(() => {
		setPrice(
			Math.floor(
				orderInfo.price -
					Number(calculateDiscount(String(orderInfo.price), orderInfo.discount || 0)),
			),
		);
	}, [orderInfo]);
					...
					<>
						...
						<ModalBody className={'flex flex-col gap-1'}>
							<Image
								...
								src={orderInfo.image}
							/>
							<div className={'flex flex-col gap-1 border-gray-200 border-b-[5px] mt-1'}>
								<p className={'font-semibold text-lg capitalize'}>{orderInfo.name}</p>
								<div className={'flex flex-row justify-between my-1'}>
									<div className={'flex flex-col'}>
										<p className={'text-default-900 font-semibold text-xl'}>
											{formatCurrencyWithDot(price)}.000đ
										</p>
									...
									<div className={'rounded-lg flex flex-row justify-center items-center'}>
										<button
											...
											className={
												'rounded text-orange-600 border-orange-600 border-2  w-fit text-xl px-3'
											}>
											-
										</button>
										<input
										...
										<button
											...
											className={
												'rounded bg-orange-600 border-orange-600 border-2 text-white w-fit text-xl px-3'
											}>
											+
										</button>
									</div>
								...
								<div>
									<p className={'text-gray-600 text-sm font-semibold'}>Thông tin thêm: </p>
									<p className={'text-gray-500 text-sm mb-2'}>{orderInfo.description}</p>
								</div>
							</div>
						...
						<ModalFooter>
							<div className={'flex flex-row gap-2 mb-2'}>
								<Button className={'bg-orange-600 text-white'} onClick={handleAddToCart}>
									Thêm vào giỏ hàng
								</Button>
								<Button className={''} onClick={handleImmediately}>
									Đặt ngay
								</Button>
							</div>
						</ModalFooter>
