...
function ShopItemCard({
	...
	return (
		<Card
			...
			key={dishID}>
			<CardBody className='overflow-hidden p-0 sendtocart'>
				<div className={'w-full h-full flex justify-center items-center bg-gray-100'}>
					<Image
						...
						width='100%'
						alt={title}
						className='w-full object-contain h-full hover:scale-110 transition-transform duration-300 ease-in-out'
						src={img}
					/>
				</div>
				...
				<div
					className={
						'absolute right-0 p-3 text-xl text-white bg-orange-600 z-[20] rounded-bl-xl hover:bg-orange-500'
					}
			...
			<CardFooter className='text-small flex-col justify-start items-start'>
				...
				<div className={'flex justify-between items-center w-full'}>
					<p className='text-default-900 text-lg font-semibold my-1'>
						{formatCurrencyWithDot(
							Math.floor(Number(price) - Number(calculateDiscount(String(price), discount))),
						)}
						.000đ
					</p>
